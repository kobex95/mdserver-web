...
if [ $OSNAME != "macos" ];then
	...
	if [ ! -d /www/server/mdserver-web ];then
		curl --insecure -sSLo /tmp/master.tar.gz ${HTTP_PREFIX}github.com/kobex95/mdserver-web/archive/refs/heads/master.tar.gz
...
if [ "${OSNAME}" == "macos" ];then
	curl --insecure -fsSL ${HTTP_PREFIX}raw.githubusercontent.com/kobex95/mdserver-web/refs/heads/master/scripts/install/macos.sh | bash
else
